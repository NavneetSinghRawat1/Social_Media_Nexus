...
import { fetchFeedPosts_global } from "../api_temp/feedApi";
import PostCard from "../components/PostCard";
// import {useContext} from 'react';
// import { UserDataContext } from "../components/contaxt/userContaxt";
export default function GlobalFeed() {
    // {console.log("Rendering GlobalFeed");}
    ...
    const loadPosts = async () => {
        ...
        try {
            console.log("Fetch global Posts");
            // const data = await fetchFeedPosts_global({
            //     cursorScore: cursor?.cursorScore,
            //     cursorPostId: cursor?.cursorPostId,
            // });
            const data = await fetchFeedPosts_global({
                // cursorCreatedAt: cursor?.cursorCreatedAt,
                cursorFeedId: cursor?.cursorFeedId,
            });
            setPosts(prev => {
                    ...
                    if (index === posts.length - 2) {
                        return (
                            <div
                                ...
                                // className="mb-4"
                            >
                                <PostCard post={post}  isowner={false}/>
                            </div>
                        );
                    }
